...
class MiniSom(object):
    ...
    def train_with_qe(self, data, num_iteration,
                      random_order=False, verbose=False,
                      use_epochs=False, fixed_points=None,
                      log_every=10):
        """Trains the SOM like `train`, but also logs quantization error.
        ...
        fixed_points : dict (default=None)
            Same meaning as in `train`.

        log_every : int (default=10)
            Compute and store the quantization error every `log_every`
            updates. If None or <= 0, QE is not logged.

        Returns
        -------
        steps : list of int
            Iteration numbers at which QE was logged.
        q_errors : list of float
            Quantization error values at the corresponding steps.
        """
        self._check_iteration_number(num_iteration)
        self._check_input_len(data)
        random_generator = None
        ...
        for t, iteration in enumerate(iterations):
            decay_rate = get_decay_rate(t, len(data))
            x = data[iteration]
            winner = fixed_points.get(iteration, self.winner(x))
            self.update(x, winner, decay_rate, num_iteration)

            if log_every is not None and log_every > 0 and (t + 1) % log_every == 0:
        ...
        return steps, q_errors
    
    def train_random_qe(self, data, num_iteration, verbose=False, log_every=50):
   
        return self.train_with_qe(data,num_iteration,random_order=True,verbose=verbose,use_epochs=False,fixed_points=None,log_every=log_every)
